...
def genFeatures3D(img, proj_matrix, box_2d, dimensions, alpha, theta_ray, img_2d=None):
    """
    ...
    bev_corners = plot_3d_box(
        img, proj_matrix, orient, dimensions, location
    )  # 3d boxes
...
def parse_opt():
    ...
    parser.add_argument(
        "--reg_weights",
        type=str,
        default="weights/resnet18.pkl",
        help="Regressor model weights",
    )
    parser.add_argument(
        "--output_path", type=str, default=ROOT / "runs/track", help="Save output pat"
    )
    parser.add_argument(
        "--video", type=str, default="testvid.mp4", help="Video file path"
    )
    parser.add_argument(
        "--calib_file",
        type=str,
        default=ROOT / "eval/camera_cal/calib.txt",
...
def video_tracker(
    ...
    return f"{output_path}/output.mp4"
